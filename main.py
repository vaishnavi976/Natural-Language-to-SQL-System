...
# Pipeline:
#   POST /chat
#     → Vanna 2.0 Agent (llama-3.3-70b-versatile + RunSqlTool)
#       The agent reads the system prompt which contains all 20 verified
#       Q→SQL examples and strict SQLite rules, then calls run_sql.
#     → SQL Validation (SELECT-only security guard)
#     → SQLite execution
#     → Plotly chart generation
#     → JSON response

#   GET /health → DB status + agent memory item count
# """
...
# # ── SQL auto-fixer ─────────────────────────────────────────────────────────────
# # Silently converts MySQL/PostgreSQL syntax to SQLite equivalents.
# # This is a thin safety net — the system prompt already instructs the model
# # to use SQLite syntax, but this catches any slip-through.
# _SQL_FIXES = [
...
#     """Auto-correct SQLite compatibility issues in LLM-generated SQL."""
#     fixed = sql
#     for pattern, replacement in _SQL_FIXES:
#         fixed = pattern.sub(replacement, fixed)
#     # Fix wrong self-table PK aliases (p.patient_id → p.id)
#     # but preserve FK join conditions (ON i.patient_id = p.id)
#     fixed = re.sub(
...
#     The agent reads the system prompt (which includes all 20 verified Q→SQL
#     examples and the full schema), then calls RunSqlTool with a SELECT query.
...
#                 f"Tip: Groq free tier allows ~30 req/min — space your queries slightly."
#             ),
...
#             log.error("DB ERROR | %s | sql=%.200s", exc, sql)
#             return ChatResponse(
#                 message=(
...
Pipeline:
  POST /chat
    ┌─ Primary: Vanna 2.0 Agent (llama-3.3-70b-versatile + RunSqlTool)
    │   The agent reads system prompt (schema + 20 verified examples + SQLite
    │   rules), then calls RunSqlTool with a SELECT query.
    │   On success → SQL validated → executed → chart → response
    │
    └─ Recovery (Groq tool-call bug fallback):
        When Groq throws "Failed to call a function" the agent crashes mid-stream.
        Any SQL extracted before the crash is partial/corrupt — we discard it.
        We then call Groq directly (no tool-calling, no streaming crash) with the
        same schema + 20 examples in the prompt. The same AI model still decides
        the SQL — just without the broken tool-calling wrapper.
        This path is logged clearly and documented in RESULTS.md.
...
async def _direct_sql(question: str) -> str | None:
    """
    Call Groq directly with NO tool-calling.
    Used when the Vanna agent throws 'Failed to call a function'.
    The same LLM still generates the SQL — just without the broken tool wrapper.
    """
...
async def _ask_vanna(question: str) -> tuple[str | None, str, list | None, list | None]:
    """
    Drive the Vanna 2.0 agent.
    Returns (sql, agent_text, cols, rows).
    Raises RateLimitError on 429.
    Raises other exceptions (including 'Failed to call a function') on hard failure.

    IMPORTANT: if this raises ANY non-rate-limit exception, the caller must
    discard any partial state and call _direct_sql() instead.
    Any SQL collected before the exception is from an incomplete agent turn
    and must NOT be used — it may reference wrong tables/columns.
    """
    from vanna_setup import get_agent
    from vanna.core.user import RequestContext
        ...
        try:
            ...
            if is_rate:
                ...
                if attempt == 0:
                    ...
                    continue
                raise RateLimitError(wait) from exc
            # For any other error (including "Failed to call a function"),
            # raise immediately. The caller will use _direct_sql() instead.
            raise
...
async def chat(request: Request, body: ChatRequest):
    ...
    try:
        ...
        # "Failed to call a function" or other Groq streaming crash.
        # DISCARD any partial SQL — it came from an incomplete agent turn
        # and will reference wrong tables/columns (as seen in logs).
        # Fall through to _direct_sql() below.
    ...
    # ── Recovery: direct Groq call (no tool-calling) ──────────────────────────
    # Only reached when the agent threw an exception.
    # Same model, same schema, same 20 examples — AI still decides the SQL.
    # No tool-calling wrapper means no "Failed to call a function" crash.
    ...
    if vanna_cols is not None and vanna_rows is not None:
        ...
        try:
            ...
            # If agent SQL failed in DB, try direct call as recovery
            if via == "agent":
